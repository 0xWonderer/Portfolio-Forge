...
export const BentoHeader: React.FC<BentoHeaderProps> = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 w-full min-w-0">
      ...
      <div className={`xl:col-span-8 p-5 sm:p-7 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 backdrop-blur-xl relative overflow-hidden group ${theme.borderHover} transition-all duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.4)] flex flex-col justify-between min-w-0`}>
        ...
        <div>
          <div className="flex items-center flex-wrap gap-2 mb-4 relative z-10">
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold ${theme.badge} shadow-sm shrink-0`}>
            ...
            {data.location && (
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs text-zinc-300 bg-zinc-800/80 border border-zinc-700/60 shrink-0">
                <MapPin className={`w-3 h-3 ${theme.text}`} />
          ...
          <div className="relative z-10 min-w-0">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white tracking-tight leading-tight mb-1.5 font-sans break-words">
              {data.name || "Alex Rivera"}
            </h1>
            <p className={`text-sm sm:text-base font-semibold text-transparent bg-clip-text bg-gradient-to-r ${theme.gradientText} mb-3 break-words`}>
      ...
      <div className={`xl:col-span-4 p-5 sm:p-7 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 backdrop-blur-xl flex flex-col justify-between ${theme.borderHover} transition-all duration-300 relative overflow-hidden group shadow-[0_10px_30px_rgba(0,0,0,0.4)] min-w-0`}>
        <div
        ...
        <div>
          <div className="text-[11px] font-mono font-bold uppercase tracking-widest text-zinc-400 mb-4 flex items-center justify-between">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-1 gap-3">
            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-850 flex items-center justify-between">
              <div>
                <div className="text-2xl font-black text-white font-mono">{data.projects.length}</div>
                <div className="text-[11px] text-zinc-400">Featured Works</div>
              </div>
            ...
            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-850 flex items-center justify-between">
              <div>
                <div className={`text-2xl font-black font-mono ${theme.text}`}>{data.skills.length}</div>
                <div className="text-[11px] text-zinc-400">Competencies</div>
              </div>
              <Shield className={`w-5 h-5 ${theme.text} shrink-0`} />
            </div>
          </div>
        </div>

        <div className="pt-3.5 border-t border-zinc-800/80 text-[11px] text-zinc-400 flex items-center justify-between font-mono mt-3">
